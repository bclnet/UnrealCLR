		...
		if (Directory.GetFiles(projectPath, "*.uproject", SearchOption.TopDirectoryOnly).Length != 0) {
			...
			if (overwriteFiles) {
				...
				var runtimeCompilation = Process.Start(new ProcessStartInfo {
					FileName = "dotnet",
					Arguments =  $"publish \"{ sourcePath }/Source/Managed/Runtime\" --configuration Release --framework net7.0 --output \"{ projectPath }/Plugins/UnrealCLR/Managed\"",
				...
				var frameworkCompilation = Process.Start(new ProcessStartInfo {
					FileName = "dotnet",
					Arguments =  $"publish \"{ sourcePath }/Source/Managed/Framework\" --configuration Release --framework net7.0 --output \"{ sourcePath }/Source/Managed/Framework/bin/Release\"",
					CreateNoWindow = false,
					UseShellExecute = false
				});
				...
				if (compileTests) {
					...
					var testsCompilation = Process.Start(new ProcessStartInfo {
						FileName = "dotnet",
						Arguments =  $"publish \"{ sourcePath }/Source/Managed/Tests\" --configuration Release --framework net7.0 --output \"{ projectPath }/Managed/Tests\"",
						CreateNoWindow = false,
						UseShellExecute = false
					});
